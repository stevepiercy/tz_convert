[define_tag('tz_convert',
            ...
            -encodenone,
            -description="tz_convert converts datetime types from one time zone locale to another for Lasso 8.x.  This is especially useful for converting datetimes to and from UTC.  Using UTC makes performing calculations with datetime types simple because it avoids issues with time zones and daylight savings time.
...
-tzin is optional and must be a valid time zone according to the tz database (see http://www.twinsun.com/tz/tz-link.htm).  The value is a user-friendly time zone representation parameter, containing a country, state, or city, e.g. 'America/Los_Angeles'.  If the time zone is not valid, then the default is UTC.  Thus to avoid unexpected results, the developer should validate the time zone prior to passing it into this tag.  It is the timezone from which the datetime will be converted.

-tzout is exactly the same as -tzin, except it is the timezone to which the datetime will be converted.

-format is optional.  If provided, -format must be an acceptable format according to 'date' in coreutils.  Defaults to '%F %T', e.g., 2013-03-31 08:00:00.  For more formats, see http://www.gnu.org/software/coreutils/manual/coreutils.html#date-invocation
